...
## intact sketches
python extract_features.py --data='../sketches/' --layer_ind=5 --data_type=sketch --spatial_avg=True --channel_norm=False --out_dir='../features/'

## instance sketches for photodraw32
python extract_features.py --data=../stimuli/photodraw32_stims_agglomerate --layer_ind=5 --data_type=image --spatial_avg=True --channel_norm=False --out_dir=../features/ 
'''
...
def save_features(Features, Y, layer_num, data_type, experiment, out_dir='features'):
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    layers = ['P1','P2','P3','P4','P5','FC6','FC7']
    np.save(os.path.join(out_dir,'FEATURES_{}_{}_{}.npy'.format(layers[int(layer_num)], experiment, data_type)), Features)
    Y.to_csv(os.path.join(out_dir,'METADATA_{}_{}.csv'.format(experiment, data_type)), index=True, index_label='feature_ind')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--experiment_flag', type=str, help='experiment name', default="photodraw2x2")
    ...
    ## break if sketch path is empty
    if len(image_paths) == 0:
        print('The input path is empty!')
        exit()
    ...
    if args.test==False:        
        layer = save_features(Features, Y, args.layer_ind, args.data_type, args.experiment_flag, out_dir = args.out_dir)  