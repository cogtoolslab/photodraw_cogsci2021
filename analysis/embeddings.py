...
## VGG19 feature dimensions by layer_ind
...
class FeatureExtractor():
                                            # if throws error, just set to False torch.cuda.is_available()
    def __init__(self,paths,layer=6, use_cuda=torch.cuda.is_available(), imsize=224, batch_size=64, cuda_device=1, data_type='images',spatial_avg=True):
        self.layer = layer
        self.paths = paths
    ...
    def extract_feature_matrix(self):
        ...
        def load_image(path, imsize=224, padding=self.padding, volatile=True, use_cuda=self.use_cuda):
            im = Image.open(path)
            if self.data_type=='sketch' or self.data_type=='image': ## if the latter condition satisfied, could be 2-dim (300,300) image
            ...
            im = Variable(loader(im_), requires_grad=False) # used to be volatile = volatile instead of requires_grad = False       
            if self.use_cuda:
                im = im.cuda(self.cuda_device)                    
                
            return im              

        def load_vgg19(layer_index=self.layer,use_cuda=self.use_cuda,cuda_device=self.cuda_device):
            if self.use_cuda:
                vgg19 = models.vgg19(pretrained=True).cuda(self.cuda_device) # https://pytorch.org/vision/0.8/models.html resnet50
            else:
        ...
        def generator(paths, imsize=self.imsize, use_cuda=self.use_cuda):
            for path in paths:
