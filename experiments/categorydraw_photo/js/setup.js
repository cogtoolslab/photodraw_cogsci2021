...
function Experiment () {
  ...
  this.colname = 'categorydraw-photo'
...
function setupGame() {
  socket.on('onConnected', function(d) {
    ...
    var practiceTrial = _.extend({}, new Experiment, {
      ...
      condition: 'photo',
      imageURL: '/categorydraw_photo/stimuli/Surprised_Pikachu.png',
      practiceTrial: true  
    });
    ...
    instructionsHTML = { 
      'str1' : '<p>In this study, you will be making drawings of various objects. Your goal is to make these drawings recognizable to someone else trying to identify what <b>kind</b> of object you were trying to draw.</p>\
      <p>For example, suppose we asked you to draw a face. Rather than drawing a specific person\'s face, you would try to draw a more <b>generic</b> face. In other words, someone looking at your drawing shouldn\'t think that it\'s of anyone in particular, only that it\'s a drawing of a face. You shouldn\'t, however, be concerned about making them look pretty.</p>\
      ...
      'str3': '<p>To help you remember what these objects look like in general, on every trial you will be shown an example, which will be on screen for 8 seconds. Then you\'ll proceed to draw a generic version of that object, but not one that looks like the example you were shown:</p>\
                <img height = "300" src = "stimuli/category_photo_screencap.gif">\
                <p>Although you will have as much time as you need to make your drawing, you won’t be able to erase or ‘undo’ any part of your drawing while you are making it. So please do your best to think about how you want your drawing to look before you begin each one. When you are satisfied with the drawing, please click SUBMIT.</p>',
    ...
    var loopNode = {
      ...
      loop_function: function(data) {
          ...
          if ((resp["bannedDrawings"] == 'No' 
            && resp["goalOfDrawing"] == "To make a drawing that is recognizable, but not one that could be matched to the image I was shown." 
            // && resp['categoryLevel'] == "A recognizable drawing would be identifiable as a member of the target category, but should not look like a specific instance."
            // && resp['categoryLevelPhoto'] == "No. I should make my drawing identifiable as a member of a target category, but not look like the specific image I was shown."
