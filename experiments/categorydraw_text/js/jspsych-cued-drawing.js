  ...
  plugin.trial = function(display_element, trial) {    
    ...
    function show_cue() {    
      ...
      if (trial.practiceTrial === true) {
        html += '<div id="occluder"><p style="padding-top: 250px;padding-bottom: 300px;">Think of the most generic <b>FACE</b> you can!</p>'//'<div id="occluder"><p style="padding-top: 200px;padding-bottom: 300px;">This is a practice trial! Before each trial, you will be asked to visualize an object<br><br>In this case, visualize anything you want! <br><br> If the layout looks off, please resize your browser accordingly!</p> </div>'
      } else if (trial.category === "bread") {
        html += '<div id="occluder"><p style="padding-top: 250px;padding-bottom: 300px;">Think of the most generic piece of <b>'+ trial.category.toUpperCase()+ '</b> you can!</p>'
      } else {
        //a_or_an = ['a','e','i','o','u'].includes(trial.category[0])      + (a_or_an ? 'an <b>' : 'a <b>')
        html += '<div id="occluder"><p style="padding-top: 250px;padding-bottom: 300px;">Think of the most generic <b>' + (trial.category === 'car_(sedan)' ? 'car (sedan)' : trial.category).toUpperCase() + '</b> you can!</p>'
      }

      // add countdown to occluder
      var i = 0;
      function move() {
        if (i == 0) {
          i = 1;
          ...
          function frame() {
            if (width <= 0) {
              ...
              elem.innerHTML = Math.round(width/125) + " s";
            }
          }
        }
      }
      html += '<div id="myProgress"><div id="myBar">8 s</div></div>'
      html += '</div>'
      ...
      // count down on load
      myProgress.addEventListener('load', move())
      ...
      if (trial.practiceTrial === true) {
        jsPsych.pluginAPI.setTimeout(function() {$('#occluder').fadeOut('fast');},8000);
      } else {
        jsPsych.pluginAPI.setTimeout(function() {$('#occluder').fadeOut('fast');},8000);
      }
