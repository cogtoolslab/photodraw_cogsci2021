...
function serve() {
  ...
  mongoConnectWithRetry(2000, (connection) => {
    ...
    app.post('/db/getIndex', (request, response) => {
      if (!request.body) {
        return failure(response, '/db/getIndex needs post request body');
      }
      console.log(`got request to get index from ${request.body.dbname}/${request.body.colname}`);

      const databaseName = request.body.dbname;
      const collectionName = request.body.colname;
      if (!collectionName) {
        return failure(response, '/db/getIndex needs collection');
      }
      if (!databaseName) {
        return failure(response, '/db/getIndex needs database');
      }

      const database = connection.db(databaseName);
      const collection = database.collection(collectionName);

      // find an element which hasn't been marked yet
